...
/**
 * Queen piece in a game of chess
 * 
...
public class Queen extends ChessPiece {
     ...
     * 
     */
    public Queen(char c, Color color, int x, int y, int movesX, int movesY) {
        super('Q', color, x, y, y, y);
        this.color = color;
        this.x = x;
        this.y = y;
    }
    ...
    boolean validMove(myButton[][] buttons, int x, int y, int movesX, int movesY) {
        ...
        if (Math.abs(movesX) >= 1 && Math.abs(movesX) < 8 && Math.abs(y) < 8 && Math.abs(movesY) >= 1) {
            ...
            if (isValid) {
                ...
                if(a == null && (x == movesX || y == movesY) || (Math.abs(x-movesX) ==Math.abs(y-movesY))){
                    isValid= true;
                }
              
                // check the destination pieces colors
                if (a != null && a.color != buttons[x][y].color && (x == movesX || y == movesY) || (Math.abs(x-movesX) ==Math.abs(y-movesY))) {
                    isValid = true;
                }
            }
        }
